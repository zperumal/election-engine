...
"""
    ...
    # This is necessary when using pypy, because there's no scipy    pass
    pass
import numpy
"""
##############################################################################
...
def prefs(B):
    ...
    for ballot in B:
        assert sorted(ballot) == list(range(m))

    A = [ [0 for j in range(m)] for i in list(range(m)) ]
    for ballot in B:
...
def generate_tweaked_profile(A, n, printing_wanted=True):
    ...
    return generate_profile(A, n, printing_wanted)
...
def sample(B, n, printing_wanted=False):
    """ 
    ...
    if printing_wanted:
        print(A)
    return generate_tweaked_profile(A, n,printing_wanted)
